...
public class CadastroFuncionario {
	...
	public CadastroFuncionario(Empregador usuario) {
		initialize(usuario);
	}
	...
	private JFormattedTextField salarioCaixa;
	private JLabel nomeLabel;
	private JLabel cpfLabel;
	private JLabel cadastroLabel;
	private JLabel dadosLabel;
	private JButton cadastrarButton, voltarButton, outroDescontoButton;
	...
	private void initialize(Empregador usuario) {
		...
		frame.setTitle("• Rabisco Holerite| Cadastrar Funcionario");
		frame.setIconImage(
				Toolkit.getDefaultToolkit().getImage(CadastroFuncionario.class.getResource("/imagem/rabisco.png")));
		...
		voltarButton.setIcon(new ImageIcon(CadastroFuncionario.class.getResource("/imagem/voltar.png")));
		...
		cadastroLabel = new JLabel("Cadastro de Funcionario");
		...
		salarioCaixa = new JFormattedTextField();
		...
		table.setModel(new DefaultTableModel(
				new Object[][] { { "001", "Salario Base", null, null, null }, { "002", "IRRF", null, null, null },
						{ "003", "INSS", null, null, null }, { "004", "FGTS", null, null, null }, },
		...
		outroDescontoButton.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				...
				if (model.getRowCount() == 10) {
					...
					JFrame frameDesconto = new JFrame();
					frameDesconto.getContentPane().setFont(new Font("Dialog", Font.BOLD, 12));
					frameDesconto.setTitle("• Rabisco Holerite| Cadastrar Funcionario");
					frameDesconto.setIconImage(Toolkit.getDefaultToolkit()
							.getImage(CadastroFuncionario.class.getResource("/imagem/rabisco.png")));
					frameDesconto.getContentPane().setBackground(Color.WHITE);
					frameDesconto.setBounds(100, 100, 400, 370);
					frameDesconto.setDefaultCloseOperation(JFrame.EXIT_ON_CLOSE);
					frameDesconto.getContentPane().setLayout(null);
					...
					frameDesconto.getContentPane().add(idCaixa);
					...
					frameDesconto.getContentPane().add(nomeCaixaDes);
					...
					frameDesconto.getContentPane().add(referenciaCaixa);
					...
					frameDesconto.getContentPane().add(vencimentoCaixa);
					...
					frameDesconto.getContentPane().add(descontoCaixa);
					...
					frameDesconto.getContentPane().add(salvarButton);
					...
					frameDesconto.getContentPane().add(cancelarButton);
					...
					frameDesconto.getContentPane().add(lblNewLabel);

					JLabel lblNome = new JLabel("Nome:");
					lblNome.setFont(new Font("Arial", Font.PLAIN, 12));
					lblNome.setBounds(55, 113, 117, 25);
					frameDesconto.getContentPane().add(lblNome);
					...
					frameDesconto.getContentPane().add(lblReferncia);
					...
					frameDesconto.getContentPane().add(lblVencimento);

					JLabel lblDesconto = new JLabel("Desconto:");
					lblDesconto.setFont(new Font("Arial", Font.PLAIN, 12));
					lblDesconto.setBounds(55, 221, 117, 25);
					frameDesconto.getContentPane().add(lblDesconto);
					...
					frameDesconto.getContentPane().add(lblNovoDesconto);
					...
					frameDesconto.setLocationRelativeTo(null);
					frameDesconto.setResizable(false);
					frameDesconto.setVisible(true);
				}
			}
		});
		painelDescontos.add(outroDescontoButton);
		...
		cadastrarButton.setBounds(189, 985, 200, 45);
		...
		cadastrarButton.addActionListener(e -> {
			...
			try {
				cpfExiste = funcionarioDao.cpfCadastrado(cpf);
			...
			if (nome.isEmpty() || cpf.isEmpty() || cbo.isEmpty() || funcao.isEmpty() || salario.isEmpty()
				...
				if (cpfV == false || cboV == false || cpfExiste == true) {
					if (cpfV == false) {
					...
					int numHolerite = generateRandomNumberFromCPF(cpf);
					...
					try {
						...
						Holerite holerite = new Holerite(numHolerite, listaDescontos, mensagem, totVencimento,
						...
						holeriteDAO.criarHolerite(holerite);
						new CAE_Holerite(usuario);
						frame.dispose();
	...
	public static void caixaOk(String texto) {
		JFrame caixaDialogo = new JFrame();
		caixaDialogo.setIconImage(
				Toolkit.getDefaultToolkit().getImage(CadastroFuncionario.class.getResource("/imagem/java.png")));
		caixaDialogo.getContentPane().setBackground(Color.WHITE);
		caixaDialogo.setBounds(100, 100, 400, 200);
		caixaDialogo.setLocationRelativeTo(null);
		caixaDialogo.setVisible(true);
	...
	public int generateRandomNumberFromCPF(String cpf) {
		String cpfWithoutCheckDigits = cpf.substring(0, cpf.length() - 2);
		int sum = 0;
		for (int i = 0; i < cpfWithoutCheckDigits.length(); i++) {
			char digit = cpfWithoutCheckDigits.charAt(i);
			sum += Character.getNumericValue(digit);
		}
		int multiplier = 7;
		int result = sum * multiplier;
		int randomNumber = result % 10000;
